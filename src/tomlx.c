...
char *tomlx_array_to_string(toml_array_t *ta, int print_syntax)
{
    ...
    for (int i = 0; i < idx_ct; i++) {
        ...
        if (i > 0) {
            if ((char_ct + 3) > bufsz) {
                fprintf(stderr, "realloc for comma");
            }
            else if (!empty) {
                errno = 0;
        ...
        if (typ == TOML_NONDATUM) {
            ...
            case TOML_ARRAY:
                ...
                /* } else { */
                    ...
                    if ((char_ct + len) > bufsz) {
                        fprintf(stderr, "exceeded bufsz: %lu", char_ct + len);
                        // expand buf
                    }
                    errno = 0;
            ...
            case TOML_TABLE:
                ...
                break;
            default:
                fprintf(stderr, "Bad toml seq type: %d", typ);
            }
        } else {
            switch(typ) {
            case TOML_INT:
                TOMLX_LOG_DEBUG("toml datum val: %d", datum.u.i);
                len = snprintf(NULL, 0, "%" PRId64, datum.u.i);
                ...
                ct = snprintf(buf+char_ct, len, "%"PRId64, datum.u.i);
                if (errno) {
                    fprintf(stderr, "snprintf: %s", strerror(errno));
                    break;
                } else {
            ...
            case TOML_DOUBLE:
                ...
                break;
            case TOML_TIMESTAMP:
                fprintf(stderr, "toml timestamp (not yet)");
                if ((char_ct + 18) > bufsz) {
                    // realloc
                } else {
                    errno = 0;
            ...
            case TOML_NONDATUM:
                // should not happen
                fprintf(stderr, "Unexpected TOML_NON_DATUM");
                //FIXME: throw error
                return NULL;
                break;
            default:
                fprintf(stderr, "Bad toml_datum constant: %d", typ);
                //FIXME: throw error
                return NULL;
            }
        }
...
char *tomlx_table_to_string(toml_table_t *tt, int print_syntax)
{
    ...
    for (int i = 0; i < key_ct; i++) {
        k = toml_key_in(tt, i);
        if (!k) {
            fprintf(stderr, "toml_key_in failure for idx: %d", i);
            return NULL;
        }
...
char *tomlx_format_datetime(toml_timestamp_t* ts, const char *fmt)
{
    ...
    /* if (fmt[0] != '$') { */
    /*     fprintf(stderr, "Datetime format string missing initial '$': %s", fmt); */
    /* } */
    static char buf[512];
    char *bufptr = buf;
    ...
    while (*p) {
        if (*p == '$') {
            ...
            case 'd':
                ...
                break;
            default:
                fprintf(stderr, "Bad date formatting code: %s", p);
                break;
            }
            p++;
            continue;
                                     ...
                                     const char *key)
{
    ...
    datum = toml_int_in(tt, key);
    if (datum.ok) {
        /* TOMLX_LOG_DEBUG("INT: % "%" PRId64, datum.u.i); */
        struct tomlx_item_s *rv = tomlx_make_item(&datum.u.i, TOML_INT);
        /* TOMLX_LOG_DEBUG("INT x: %l "%" PRId64, rv->u.i); */
        return rv;
    }
    // BOOL
    datum = toml_bool_in(tt, key);
                                           ...
                                           int idx)
{
    ...
    if (datum.ok) {
        /* TOMLX_LOG_DEBUG("INT: "%" PRId64, datum.u.i); */
        struct tomlx_item_s *rv = tomlx_make_item(&datum.u.i, TOML_INT);
        /* TOMLX_LOG_DEBUG("INT x: "%" PRId64, rv->u.i); */
        return rv;
    }
    // BOOL
    datum = toml_bool_at(ta, idx);
...
toml_table_t *tomlx_read_string(const char *s)
{
    ...
    if (tt == NULL) {
        fprintf(stderr, "toml:read failure: %s", errbuff);
        return NULL;
   }
    return tt;
}
